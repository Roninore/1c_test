...
Процедура ПрочитатьФайл_XLSX()    
	...
	Сообщить("Начинаю помещать файл");
...
Процедура ЗавершениеОбратныйВызов(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт  
	...
	Строки = ПрочитатьФайл_XLSX_НаСервере(ОписаниеПомещенногоФайла.Адрес);       
	Сообщить("Массив получен" + Строки.Количество());     
	Для Каждого Строка ИЗ Строки Цикл
		СтрокаДанных = Объект.ДанныеФайла.Добавить();
		СтрокаДанных.ФИО = Строка.ФИО;
		СтрокаДанных.ЛичноеДело = Строка.ЛичноеДело;
		СтрокаДанных.ДатаРождения = Строка.ДатаРождения;
		СтрокаДанных.Пол = Строка.Пол;
		СтрокаДанных.Адрес = Строка.Адрес;
		СтрокаДанных.ПФДО = Строка.ПФДО;
		СтрокаДанных.Документ = Строка.Документ;
		СтрокаДанных.ДокументСерия = Строка.ДокументСерия;
		СтрокаДанных.ДокументНомер = Строка.ДокументНомер;
		СтрокаДанных.ДатаВыдачиДокумента = Строка.ДатаВыдачиДокумента;
		СтрокаДанных.ДокументВыдан = Строка.ДокументВыдан;
		СтрокаДанных.СНИЛС = Строка.СНИЛС;
		СтрокаДанных.РодительФИО = Строка.РодительФИО;
		СтрокаДанных.РодительДатаРождения = Строка.РодительДатаРождения;
		СтрокаДанных.РодительПол = Строка.РодительПол;
		СтрокаДанных.РодительТип = Строка.РодительТип;
		СтрокаДанных.РодительДокументСерия = Строка.РодительДокументСерия;
		СтрокаДанных.РодительДокументНомер = Строка.РодительДокументНомер;
		СтрокаДанных.РодительДокументДатаВыдачи = Строка.РодительДокументДатаВыдачи;
		СтрокаДанных.РодительДокументВыдан = Строка.РодительДокументВыдан;
		СтрокаДанных.РодительСНИЛС = Строка.РодительСНИЛС;
		СтрокаДанных.РодительТелефон = Строка.РодительТелефон;
		СтрокаДанных.ЗачислениеДатаЗаполнения = Строка.ЗачислениеДатаЗаполнения;
		СтрокаДанных.ЗачислениеДатаДействия = Строка.ЗачислениеДатаДействия;
		СтрокаДанных.ЗачислениеКласс = Строка.ЗачислениеКласс;
		СтрокаДанных.ЗачислениеНомерПриказа = Строка.ЗачислениеНомерПриказа;
		СтрокаДанных.ПереводДатаЗаполнения = Строка.ПереводДатаЗаполнения;
		СтрокаДанных.ПереводДатаДействия = Строка.ПереводДатаДействия;
		СтрокаДанных.ПереводКласс = Строка.ПереводКласс;
		СтрокаДанных.ПереводНомерПриказа = Строка.ПереводНомерПриказа;
		СтрокаДанных.ПереводТипДвижения = Строка.ПереводТипДвижения;
	КонецЦикла;
КонецПроцедуры
...
//Заполнение из файла
&НаСервере
Функция ПрочитатьФайл_XLSX_НаСервере(Адрес)  
	Сообщить("Начало на сервере");
	...
	Для НомерСтроки=2 По КоличествоСтрок Цикл
		СтрокаПроверки = ТабДок.ПолучитьОбласть("R"+Формат(НомерСтроки,"ЧГ=0")+"C"+1).ТекущаяОбласть.Текст;
		...
		СтрокаДанных.Вставить("ФИО", ПолучитьОбласть(ТабДок,НомерСтроки,1,""));
		СтрокаДанных.Вставить("ЛичноеДело", ПолучитьОбласть(ТабДок,НомерСтроки,2,""));
		СтрокаДанных.Вставить("ДатаРождения", ПолучитьОбласть(ТабДок,НомерСтроки,3,"Дата"));
		СтрокаДанных.Вставить("Пол", ПолучитьОбласть(ТабДок,НомерСтроки,4,"Пол"));
		СтрокаДанных.Вставить("Адрес", ПолучитьОбласть(ТабДок,НомерСтроки,5,""));
		СтрокаДанных.Вставить("ПФДО", ПолучитьОбласть(ТабДок,НомерСтроки,6,""));
		СтрокаДанных.Вставить("Документ", ПолучитьОбласть(ТабДок,НомерСтроки,7,"ТипДокумента"));
		СтрокаДанных.Вставить("ДокументСерия", ПолучитьОбласть(ТабДок,НомерСтроки,8,""));
		СтрокаДанных.Вставить("ДокументНомер", ПолучитьОбласть(ТабДок,НомерСтроки,9,""));
		СтрокаДанных.Вставить("ДатаВыдачиДокумента", ПолучитьОбласть(ТабДок,НомерСтроки,10,"Дата"));
		СтрокаДанных.Вставить("ДокументВыдан", ПолучитьОбласть(ТабДок,НомерСтроки,11,""));
		СтрокаДанных.Вставить("СНИЛС", ПолучитьОбласть(ТабДок,НомерСтроки,12,""));
		СтрокаДанных.Вставить("РодительФИО", ПолучитьОбласть(ТабДок,НомерСтроки,13,""));
		СтрокаДанных.Вставить("РодительДатаРождения", ПолучитьОбласть(ТабДок,НомерСтроки,14,"Дата"));
		СтрокаДанных.Вставить("РодительПол", ПолучитьОбласть(ТабДок,НомерСтроки,15,"Пол"));
		СтрокаДанных.Вставить("РодительТип", ПолучитьОбласть(ТабДок,НомерСтроки,16,"ТипРодителя"));
		СтрокаДанных.Вставить("РодительДокументСерия", ПолучитьОбласть(ТабДок,НомерСтроки,17,""));
		СтрокаДанных.Вставить("РодительДокументНомер", ПолучитьОбласть(ТабДок,НомерСтроки,18,""));
		СтрокаДанных.Вставить("РодительДокументДатаВыдачи", ПолучитьОбласть(ТабДок,НомерСтроки,19,"Дата"));
		СтрокаДанных.Вставить("РодительДокументВыдан", ПолучитьОбласть(ТабДок,НомерСтроки,20,""));
		СтрокаДанных.Вставить("РодительСНИЛС", ПолучитьОбласть(ТабДок,НомерСтроки,21,""));
		СтрокаДанных.Вставить("РодительТелефон", ПолучитьОбласть(ТабДок,НомерСтроки,22,""));
		СтрокаДанных.Вставить("ЗачислениеДатаЗаполнения", ПолучитьОбласть(ТабДок,НомерСтроки,23,"Дата"));
		СтрокаДанных.Вставить("ЗачислениеДатаДействия", ПолучитьОбласть(ТабДок,НомерСтроки,24,"Дата"));
		СтрокаДанных.Вставить("ЗачислениеКласс", ПолучитьОбласть(ТабДок,НомерСтроки,25,"Класс"));
		СтрокаДанных.Вставить("ЗачислениеНомерПриказа", ПолучитьОбласть(ТабДок,НомерСтроки,26,""));
		СтрокаДанных.Вставить("ПереводДатаЗаполнения", ПолучитьОбласть(ТабДок,НомерСтроки,27,"Дата"));
		СтрокаДанных.Вставить("ПереводДатаДействия", ПолучитьОбласть(ТабДок,НомерСтроки,28,"Дата"));
		СтрокаДанных.Вставить("ПереводКласс", ПолучитьОбласть(ТабДок,НомерСтроки,29,"Класс"));
		СтрокаДанных.Вставить("ПереводНомерПриказа", ПолучитьОбласть(ТабДок,НомерСтроки,30,""));
		СтрокаДанных.Вставить("ПереводТипДвижения", ПолучитьОбласть(ТабДок,НомерСтроки,31,"ТипДвижения"));    
...
Функция ПолучитьКласс(Строка)                                       
	...
	НомерКласса = Число(Части[0]);
	ЛитераКласса = Части[1];
...
Функция ПолучитьНомерДокумента(Строка)
	...
	КонецЕсли;
	Попытка
		Номер.Число = Число(Результат[0]);
		Номер.Литера = Результат[1];
	Исключение
		Номер.Число = -1;
		Номер.Литера = "";		
	КонецПопытки;         
